...
import { decrypt, encrypt, hashPhone, normalizePhone } from '../util/encryption.util';
...
export class UserModel {
    ...
    static async crearUsuario({ usuario }: { usuario: Omit<User, 'id_usuario'> }) {
        ...
        const telefonoNormalizado = normalizePhone(usuario.telefono);
        ...
        const [usuarioCreado] = await pool.execute<UserQuery[]>(
            `SELECT id_usuario, nombre_usuario, nombre_completo, correo_electronico, fecha_creacion, rol
             FROM usuarios
